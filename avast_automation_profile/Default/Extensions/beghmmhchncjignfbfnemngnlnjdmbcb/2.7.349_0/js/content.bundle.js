...
ExtensionStorage.getSync(StorageKeys.AppSettings.KEY).then(function (config) {
  ...
  if (!isDisabled) {
    ...
    args = {
      ...
      HookedFunctionsMap: HookedFunctionsMap,
      disabledFeatures: disabledFeatures,
      canvasFp: canvasFp,
      webglFp: webglFP,
      plugins: pluginsFp,
      fpAttr: FingerprintAttributes(currentProfile),
      debug: debugMode,
      t0: t0
    };
    chrome.runtime.sendMessage({
      type: 'NEW_FRAME_INJECTED',
      payload: _objectSpread({}, args)
    });
  } else {
...
document.addEventListener("fingerprintAttemptDetected", function (data) {
  chrome.runtime.sendMessage({
    type: "fingerprintAttemptDetected",
    payload: {
      type: "basic",
