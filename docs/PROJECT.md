...
```
[영림 OMS] → 자동 다운로드 (30분마다)
    ↓
[v10_state.json] READY 상태 파일 누적
    ↓
[대시보드] 견적 업로드 버튼 클릭 (수동)
    ↓
[Google Sheets Sheet1/Sheet2] 데이터 기록 (최대 300행, 초과 시 30분 후 자동 재업로드)
    ↓
[구글 시트 복사버튼] 클릭 → 30초 대기 → Ctrl+C → ERP 붙여넣기 (수동)
    ↓
[Sheet3/Sheet4 백업 + Sheet5 로그 + Sheet10/11 미매핑 저장] 자동 클리어
```
...
2. 30분마다 자동 다운로드 (영림 OMS → 로컬 저장)
3. 대시보드(`http://localhost:5080`) → **[견적 업로드]** 클릭
4. 구글 시트 Sheet1 확인 → **복사버튼** 클릭
5. 30초 안에 **Ctrl+C** → 이카운트에 **붙여넣기** → 저장(F8)
