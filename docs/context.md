...
- **추가 상태**: 5/19 장애 복구 완료 — 좀비 `msedgedriver` + lock/pid 잔존으로 인한 불완전 재시작 버그 재확인.
- **주요 스택**: 
...
3. **재시작 표준 절차 자동화**:
   - 재시작 표준 절차를 `START.bat` 또는 별도 `RESTART_CLEAN.bat`으로 자동화.
...
## 6. 2026-05-19 장애 기록: 불완전 재시작으로 인한 서버 사망

### 상황 요약
- 서버 재시작 1차 시도: READY 6건 처리 후 서버 사망.
- 원인: 좀비 `msedgedriver`(PID 4792/85344) + lock/pid 파일 잔존 상태에서 재시작.
- 완전 정리 후 재시작: 8파일 / 109행 업로드 완료.

### 확인된 원인
- lock/pid 파일(`v11.lock`, `run_server.pid`, `edge_9333.pid`) 미제거 상태 재시작.
- 좀비 `msedgedriver` 프로세스 간섭.

### 복구 순서 (표준 절차로 확정)
1. `cmd /c STOP_SCHEDULED.bat`
2. `Get-Process msedgedriver | Stop-Process -Force`
3. `netstat -ano | findstr ":9333"` → 해당 PID `taskkill`
4. `Remove-Item logs\v11.lock, logs\run_server.pid, logs\edge_9333.pid -Force`
5. `cmd /c START_SCHEDULED.bat`

### 교훈
- 재시작 전 반드시 `msedgedriver` 전체 kill + lock 파일 완전 제거 필수.
- `STOP_SCHEDULED`만으로는 부족 — 잔여 프로세스 수동 정리 필요.

## 7. 2026-06-01 � ���� �м� �� ��ġ

### �ٽ� ���
- `5/30 08:31` ��ġ�� ���� ���а� �ƴ϶� ���� ó���Ǿ���.
- �ݺ������� ���� "��ħ ����Ŭ �� �ڵ� �ٿ�ε�/���ε尡 �� �ǰ� ���� ��⵿���� �����Ǵ� ����"�� �ٽ� ������ �ٿ�ε� �������� � ���μ��� ���� �����̴�.
- Ư�� ���� 3������ ��ģ ���·� �Ǵ��Ѵ�.
  - �ߺ� `run_server.py` �ν��Ͻ� ����
...
- `logs/app_20260530.json`
  - `2026-05-30 08:31:26` �ű� 3�� �ٿ�ε� �Ϸ�
  - `2026-05-30 08:31:33` `Staged 82 rows`
  - `2026-05-30 08:31:36` �ش� 3�� `COMPLETED`
- `run_server.py` sleep heartbeat �߰� �� Ȯ�� ���
  - `sleep` ���� ��ü�� ��� �־���, 60�� heartbeat �αװ� ��� ���Ҵ�.
  - ���� "07:00 �Ϸ� �� sleep ��ü�� ����"�� ���� �������� ���� ��ƴ�.
- ���� ��¥ �α� �ȿ� ���� �ٸ� �ν��Ͻ� ������ ������.
...
### �Է� ���â ���� �Ǵ�
- ����ڰ� �� `����Ϸ��� �ƹ� Ű�� �����ʽÿ� . . .` â�� ���ΰ� �����ϴٰ� ���� ��ƴ�.
- �ڵ� ������ ����� `START_SCHEDULED.bat`, `STOP_SCHEDULED.bat`, ���� `START.bat`���� ���� `pause`�� ������.
- �ٸ� ������Ʈ �� ����/���� ��ġ���� `pause`, `choice`, `Read-Host`�� �ټ� �����߰�, `cmd.exe` ���μ����� ������������ ���� �� �־���.
- ���� �Է� ���â�� "�ٺ� ����"�̶�⺸�� �Ʒ� �� ���� �� �ϳ��� ������ ����/���� �������� ����.
  - ����/���� ��ġ�� � ��ο� ���� ��� ���·� ����
  - ���� ���μ��� ���� �� �ٱ� cmd â�� ���� �Է� ���
...
- ����/���� ��ġ �Է� ��� �⺻ ��Ȱ��ȭ
  ...
  - `run_v10_server.bat`
  - `kill_processes.bat`
- �� ��ġ���� �⺻������ �Է� ��⸦ ���� �ʰ�, �ʿ� �ÿ��� `MANUAL_PROMPT=1`�� �� ����ϵ��� ����
- stale ���� ���� �� ��⵿ ����
  - �� ���� PID: `91824`
  - �� lock PID: `91824`
  - Edge PID: `223448`
  - `127.0.0.1:5081` listen Ȯ��
  - `127.0.0.1:9333` listen Ȯ��
  - `2026-06-01 16:41:35 ~ 16:41:43` ù ����Ŭ ���� �Ϸ� Ȯ��

### ���� �Ǵ�
- ���� ������ ���� �ν��Ͻ��� ��⵿�� �� ���� ���� ���̴�.
- �ٸ� �ݺ� ������ �ٺ� ������ ������ "�ߺ� �ν��Ͻ� + stale lock/pid + ���� 9333 ����" �࿡ �����Ƿ�, ���� ��ħ �αױ��� Ȯ���ؾ� ��� ���� ȿ���� ���� �Ǵ��� �� �ִ�.

### 2026-06-02 �߰� ����
- `START_SCHEDULED.bat` �� `run_server.pid` �� ���� ���� ���� ���θ� �Ǵ��ϸ�, PID ������ stale �̰ų� ������ ��� `5081` �� �̹� ��� �ִ� ������ �־ �� �ν��Ͻ��� �� ��� �� ������ Ȯ���ߴ�.
- �̿� ���� `START_SCHEDULED.bat` �� �����ߴ�.
  - `run_server.pid` Ȯ�κ��� ���� `127.0.0.1:5081` ������ PID�� ��ȸ
  - `5081` �� �̹� listen ���̸� �ش� PID�� `logs/run_server.pid` �� �ٽ� ����ϰ� ��⵿�� ��ŵ
  - PID ���� �б�� `set /p < file` ��� `for /f` �� ����
- `STOP_SCHEDULED.bat` �� �����ߴ�.
  - PID ������ ���ų� Ʋ���� `127.0.0.1:5081` ������ PID�� fallback ���� ã�� ���� �õ�
  - `edge_9333.pid` �� �����ϰ� `for /f` �� �е��� ����
- ���� `cmd /c START_SCHEDULED.bat` ���� �� `Input redirection is not supported` �޽����� ��Ÿ������, ���� �����ٷ� ��ο����� `2026-06-02 07:08` ������ �Ϸ�� ���� `scheduler_20260602.log` �� Ȯ���ߴ�.
- ���� �� �޽����� ���� Codex ���� ȯ���� ���ȭ�� stdin Ư�� ���� ���ɼ��� ����, ���� � ��ֿʹ� �и��ؼ� ���� �Ѵ�.
- �ٸ� ���� �α״� �̹� �ߺ� �ν��Ͻ� ������ ����ڽ� ���� ���� ������ ���� �־�, ���� û�� ������ ���� ��ħ �α׿��� ��Ȯ�� �ʿ��ϴ�.
